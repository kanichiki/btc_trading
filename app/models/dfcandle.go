...
func (df *DataFrameCandle) BackTestMacd(fastPeriod, slowPeriod, signalPeriod int) *SignalEvents {
	lenCandles := len(df.Candles)
	if lenCandles <= fastPeriod || lenCandles <= slowPeriod || lenCandles <= signalPeriod {
		return nil
	}
	signalEvents := NewSignalEvents()
	outMacd, outMacdSignal, _ := talib.Macd(df.Closes(), fastPeriod, slowPeriod, signalPeriod)
	for i := 1; i < lenCandles; i++ {
		// ゴールデンクロス
		if outMacd[i] < 0 &&
			outMacdSignal[i] < 0 &&
...
func (df *DataFrameCandle) OptimizeMacd() (performance float64, bestFastPeriod, bestSlowPeriod, bestSignalPeriod int) {
	bestFastPeriod = 12
	bestSlowPeriod = 26
	bestSignalPeriod = 9
...
func (df *DataFrameCandle) OptimizeRsi() (performance float64, bestPeriod int, bestBuyThread, bestSellThread float64) {
	...
	for period := 5; period < 25; period++ {
		for buyThread := 27.0; buyThread < 33; buyThread++ {
			for sellThread := 67.0; sellThread < 73; sellThread++ {
