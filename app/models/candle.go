...
func GetCandle(productCode string, duration time.Duration, dateTime time.Time) *Candle {
	...
	cmd := fmt.Sprintf("SELECT time, open, close, high, low,volume FROM %s WHERE time = '%s'", tableName, dateTime.Format(time.RFC3339))
...
// CreateCandleWithDuration - キャンドルを作成
// すでに存在する場合は更新
